...
if( ! function_exists( 'vmag_tabbed_title' ) ):
	function vmag_tabbed_title( $tabbed_title, $vmag_cat_id ) {
		if( !empty( $tabbed_title ) ) {
			...
			echo esc_html(get_cat_name( $vmag_cat_id ));
		}
	}
endif;
/*--------------------------------------------------------------------------------------------------------*/
...
?>
			<span class="view-all"><a href="<?php echo esc_url( $vmag_block_cat_link ); ?>" title="<?php esc_attr_e( 'View all', 'vmag' ); ?>"><i class="fa <?php echo esc_attr( $vmag_all_icon_value ); ?>"></i></a></span>
<?php
		}
	}
endif;
